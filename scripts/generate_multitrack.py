"""Multi-track generation entry.

Usage:
  CUDA_VISIBLE_DEVICES=0 python -m scripts.generate_multitrack \\
...
def main():
    p = argparse.ArgumentParser(
        description='Generate multi-track MIDI from scratch with a trained BEAT model.')
    ...
    p.add_argument('--output_dir', type=str, default='samples/multitrack',
    ...
    for i in range(args.num_samples):
        ...
        path = os.path.join(out_dir, f'sample_{i+1:03d}.mid')
        decoder.to_midi(tokens, path)
        print(f"  [{i+1}/{args.num_samples}] {len(tokens)} tokens → {path}")
