"""Multi-track training entry — trains the unified BEAT backbone on LMD multi-track data.

NOTE: No multi-track checkpoint exists yet against the unified 593-token vocab.
The legacy `Ours_multi/checkpoints/steps_40000_*` was trained against a
different vocabulary and is NOT compatible. Use this script to train a fresh
multi-track checkpoint.

Usage:
  # single GPU
  CUDA_VISIBLE_DEVICES=0 python -m scripts.train_multitrack
  ...
  # resume
  python -m scripts.train_multitrack --resume checkpoints/multitrack/backbone_best.pt

Tensorboard:
  tensorboard --logdir logs/multitrack --port 6006

Encoding differences vs. the legacy Ours_multi tokenizer (intentional):
  - descending PIT sort (paper App A.2 convention)
  - REST token for empty beats
  - VEL = DEFAULT_VEL (64) sentinel after every PAT
  - INS_DRUM single token at id 504
  - 15-bucket TEM tokens instead of 4 coarse BPM buckets
  - all token IDs shifted to UnifiedVocab layout
"""
...
from multitrack.dataset import BackboneCollator, MultitrackDataset
from multitrack.tokenizer import MultitrackTokenizer


def evaluate(model, loader, max_batches: int = 50) -> dict:
    """avg loss + perplexity over up to `max_batches` batches."""
    model.eval()
    ...
    return {'loss': avg, 'perplexity': ppl, 'batches': n}
...
def main():
    ...
    run_name = args.run_name or f"multitrack_{datetime.now().strftime('%m%d_%H%M')}"
